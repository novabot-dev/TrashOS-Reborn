...
def _write_api_status(status):
    with open(STATUS_API_FILE, "w", encoding="utf-8") as f:
        json.dump(status, f, indent=4)


def _read_api_status():
    global _cached_status, _last_mtime
    if not os.path.exists(STATUS_API_FILE):
        _write_api_status({"version": _cached_status.get("version", "1.0.0"), "is_root": bool(_cached_status.get("is_root", False))})
    try:
